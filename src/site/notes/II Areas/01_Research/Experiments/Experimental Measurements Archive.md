---
{"dg-publish":true,"permalink":"/ii-areas/01-research/experiments/experimental-measurements-archive/","title":"Experimental Measurements Archive","tags":["type/study","context/research","theme/optical-diagnostics","status/in-progress"],"noteIcon":"","created":"2026-09-01","updated":"2026-09-14","dg-note-properties":{"title":"Experimental Measurements Archive","aliases":["Experimental Measurements Archive"],"tags":["type/study","context/research","theme/optical-diagnostics","status/in-progress"],"date":"2026-09-01","last_updated":"2026-09-14"}}
---


# Experimental Measurements Archive

## Confirmed provenance and deferred counts

Seven voltage-reduction series originate in the Master's work of **May 2024**, were used in ICOLSE 2024, and are reused as APL 2025 series 1–7. Three additional delay series originate in **February 2025** and appear as APL series 8–10. These are ten series across two acquisition periods, not two independent sets of ten new doctoral experiments. The number of attempts/discharges in each series remains **unverified, to be resolved by the author**; a series is not a single shot.

The general claim that increasing irradiance shortens delay is **not adopted pending verification**. Historical electrical-trigger delay is not equated with calibrated optical-arrival-to-breakdown delay. The new doctoral validation and TCO remain planned work.

| Acquisition | Series / reuse | Attempts and discharges | Evidence status |
| --- | --- | --- | --- |
| May 2024 | Seven Master's voltage series; ICOLSE 2024 and APL 1–7 | Unverified; author to resolve | Historical provenance confirmed; raw-data/QC manifest still required |
| February 2025 | Three additional delay series; APL 8–10 | Unverified; author to resolve | Historical provenance confirmed; raw-data/QC manifest still required |
| Planned 2027 | New atmospheric baseline/pilot/confirmation, WP1–WP4 | Determined from pilot precision and frozen design | Planned, not acquired |

The old entries with 500/350/600 discharges, April 2024, a dielectric barrier and fixed pulse energies were unsupported catalogue drafts and have been withdrawn from the active table. Do not propagate them as measurement facts. Apparatus and conditions must be taken from the original acquisition record and the reviewer report with their evidence limits.

## Scope

The broader application context is the transition toward SF₆-free and, where feasible, F-gas-free high-voltage switching. The selected dissertation remains **Tier 1: atmospheric-air metrology, stochastic prediction and bounded TCO**, with **submission targeted for August 2028**. Tier 1 studies laser-triggered closure of a laboratory gap; it does not test the insulation, short-circuit interruption, post-arc recovery or lifetime required to qualify complete switchgear. Any relevance to SF₆ substitution is therefore conditional on later validation for a defined switching function and duty. Tier 2 and Tier 3 are separately resourced follow-on research outside mandatory completion.

Working authority: [[I Projects/03_Milestones/20260925 Minimum/Doctoral Progress Review and Research Plan for Professional Discussion 2026\|Doctoral Progress Review & Dissertation Plan (2024–2028)]], revised 14 September 2026. This records the candidate's planning choice; formal supervisor, committee and ISP/KOS approval remains separately evidenced.

## Evidence records

- [[II Areas/01_Research/Experiments/Dataset Manifests/Historical Measurements 2024-2025 - Provenance\|Historical Measurements 2024-2025 - Provenance]]
- [[II Areas/01_Research/Experiments/Analysis Records/Historical Counts and Delay Trend - Verification Pending\|Historical Counts and Delay Trend - Verification Pending]]
- [[II Areas/01_Research/Experiments/Evidence Index\|Evidence Index]]
- [[I Projects/02_Campaigns/Experimental Diagnostics Campaign\|Experimental Diagnostics Campaign]]
