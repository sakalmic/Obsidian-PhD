...
function getPermalinkMeta(note, key) {
  let permalink = note.url || (note.data && note.data.permalink) || "/";
  ...
  let folders = null;
  try {
    if (note.data && note.data.permalink) {
      permalink = note.data.permalink;
    }
    if (note.data && note.data.tags && note.data.tags.indexOf("gardenEntry") != -1) {
      permalink = "/";
    }    
    ...
    if (note.data && (note.data.hide || note.data.hideInFiletree)) {
      hide = true;
    }
